...
/**
 * Servlet implementation class SeeSscat
 */
public class SeeSscat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		id=Integer.valueOf(request.getParameter("id"));
		Souscategorie sscat = souscategoriedao.getById(id);
		
		ArrayList<Produit> colprods = produitdao.getAllBySCat(id);
		
		if(colprods.isEmpty()) {
			load=true;
			response.sendRedirect("NotFound");
		}else if(sscat.isDel()==true) {
			load=true;//si la scat est cachée alors 404.
			response.sendRedirect("NotFound");
		}else if(sscat.getCategorie().isDel()==true) {
			load=true;//Si la ca est cachée alors je ne veux pas voir la scat.
			response.sendRedirect("NotFound");
		}
		else {
		
		String titre = souscategoriedao.getById(id).getTitre();
		request.setAttribute("titre", titre);
		request.setAttribute("prods", colprods);
		
		Categorie c = sscat.getCategorie();
		...
		if(!load) {
		request.getRequestDispatcher( "seeSscat.jsp" ).forward( request, response );
		}}}
	}
